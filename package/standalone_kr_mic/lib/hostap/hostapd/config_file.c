			       ...
			       const char *buf, char *pos, int line)
{
	...
	} else if (os_strcmp(buf, "wpa_pmk") == 0) {
		extern u8 use_inserted_pmk;
		extern u8 inserted_pmk[PMK_LEN];
		if (hexstr2bin(pos, inserted_pmk, PMK_LEN) ||
		    pos[PMK_LEN * 2] != '\0') {
	...
	} else if (os_strcmp(buf, "s1g_oper_centr_freq_seg0_idx") == 0) {
		conf->s1g_oper_centr_freq_seg0_idx = atoi(pos);
	} else if (os_strcmp(buf, "s1g_mcs10") == 0) {
		conf->s1g_mcs10 = atoi(pos);
