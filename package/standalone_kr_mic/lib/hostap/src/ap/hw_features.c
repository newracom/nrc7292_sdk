	...
	for (i = 0; i < num_modes; i++) {
		...
		for (j = 0; j < feature->num_channels; j++) {
			...
			wpa_printf(MSG_MSGDUMP, "Allowed channel: mode=%d "
				   "chan=%d freq=%d.%d MHz max_tx_power=%d dBm%s",
				   feature->mode,
				   feature->channels[j].chan,
				   feature->channels[j].freq,
				   feature->channels[j].freq_fractional,
				  ...
				  int channel, int primary)
{
	...
	chan = hw_get_channel_chan(iface->current_mode, channel, NULL, NULL);
	if (!chan)
		return 0;
...
int hostapd_hw_get_freq_fractional(struct hostapd_data *hapd, int chan)
{
	return hw_get_freq_fractional(hapd->iface->current_mode, chan);
}
