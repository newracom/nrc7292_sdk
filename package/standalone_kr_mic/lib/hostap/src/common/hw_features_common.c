...
struct hostapd_channel_data * hw_get_channel_chan(struct hostapd_hw_modes *mode,
						  int chan, int *freq, int *freq_fractional)
{
	int i;
	...
	for (i = 0; i < mode->num_channels; i++) {
		...
		if (ch->chan == chan) {
			if (freq)
				*freq = ch->freq;
			if (freq_fractional)
				*freq_fractional = ch->freq_fractional;
			return ch;
		}
	}
...
int hw_get_freq(struct hostapd_hw_modes *mode, int chan)
{
	...
	hw_get_channel_chan(mode, chan, &freq, NULL);

	return freq;
}

int hw_get_freq_fractional(struct hostapd_hw_modes *mode, int chan)
{
	int freq_fractional;

	hw_get_channel_chan(mode, chan, NULL, &freq_fractional);

	return freq_fractional;
}
			      ...
			      int sec_chan)
{
	...
	p_chan = hw_get_channel_chan(mode, pri_chan, NULL, NULL);
	if (!p_chan)
		return 0;
	...
	s_chan = hw_get_channel_chan(mode, sec_chan, NULL, NULL);
	if (!s_chan)
		return 0;
...
int hostapd_set_freq_params(struct hostapd_freq_params *data,
			    ...
			    int freq, int freq_fractional, int channel, int ht_enabled,
			    int vht_enabled, int he_enabled, int s1g_enabled,
			    ...
			    struct he_capabilities *he_cap)
{
	...
	data->mode = mode;
	data->freq = freq;
	data->freq_fractional = freq_fractional;
	...
	if (data->s1g_enabled) {
		...
		/*
		 * Note: In S1G, center_segment0 is the units of 100 Khz
		 */
		switch (oper_chwidth) {
		case CHANWIDTH_4MHZ:
			if (center_segment0) {
				if (center_segment0 == channel - 1 &&
					center_segment0 == channel + 1)
						data->center_freq1 = 902 + center_segment0 * 5;
				else
					return -1;
			} else if (channel & 0x1) {
				if (channel >= 5 && channel <= 11)
						data->center_freq1 = 9020 + 8 * 5;
				else if (channel >= 13 && channel <= 19)
						data->center_freq1 = 9020 + 16 * 5;
				else if (channel >= 21 && channel <= 27)
						data->center_freq1 = 9020 + 24 * 5;
				else if (channel >= 29 && channel <= 35)
						data->center_freq1 = 9020 + 32 * 5;
				else if (channel >= 37 && channel <= 43)
						data->center_freq1 = 9020 + 40 * 5;
				else if (channel >= 45 && channel <= 50)
						data->center_freq1 = 9020 + 48 * 5;
				else
					return -1;
				data->center_freq1 /= 10;
			}
		break;
		case CHANWIDTH_2MHZ:
			if (center_segment0) {
...
int chan_pri_allowed(const struct hostapd_channel_data *chan)
{
	return !(chan->flag & HOSTAPD_CHAN_DISABLED) &&
		(chan->allowed_bw & (HOSTAPD_CHAN_WIDTH_20 |
				HOSTAPD_CHAN_WIDTH_1 | HOSTAPD_CHAN_WIDTH_2));
}
