...
static void rx_mgmt_beacon(struct wlantest *wt, const u8 *data, size_t len)
{
							   ...
							   &elems, 0) == ParseFailed)
	{
		if (bss->parse_error_reported)
			return;
		add_note(wt, MSG_INFO, "Invalid IEs in a Beacon frame from " MACSTR, MAC2STR(mgmt->sa));
...
static void rx_mgmt_s1g_beacon(struct wlantest *wt, const u8 *data, size_t len)
{
	...
	ext = (const struct ieee80211_ext *)data;
	offset = ext->variable - data;
	if (len < offset)
		return;
	bss = bss_get(wt, ext->sa);
	...
	// bss->capab_info = le_to_host16(ext->u.beacon.capab_info);
	if (ieee802_11_parse_elems(ext->variable, len - offset,
							   &elems, 0) == ParseFailed)
	{
		if (bss->parse_error_reported)
			return;
		add_note(wt, MSG_INFO, "Invalid IEs in a Beacon frame from " MACSTR, MAC2STR(ext->sa));
		bss->parse_error_reported = 1;
		return;
	}
...
static void rx_mgmt_auth(struct wlantest *wt, const u8 *data, size_t len)
{
	...
	if (len < 24 + 6)
	{
		add_note(wt, MSG_INFO, "Too short Authentication frame from " MACSTR, MAC2STR(mgmt->sa));
		return;
	}
	...
	wpa_printf(MSG_DEBUG, "AUTH " MACSTR " -> " MACSTR " (alg=%u trans=%u status=%u)",
	...
	if (alg == 0 && trans == 2 && status == 0)
	{
		if (sta->state == STATE1)
		{
			add_note(wt, MSG_DEBUG, "STA " MACSTR " moved to State 2 with " MACSTR,
...
static void deauth_all_stas(struct wlantest *wt, struct wlantest_bss *bss)
{
	...
	dl_list_for_each(sta, &bss->sta, struct wlantest_sta, list)
	{
		if (sta->state == STATE1)
			continue;
		add_note(wt, MSG_DEBUG, "STA " MACSTR " moved to State 1 with " MACSTR,
				 MAC2STR(sta->addr), MAC2STR(bss->bssid));
		sta->state = STATE1;
	}
}
						   ...
						   int valid)
{
	...
	if (len < 24 + 2)
	{
		add_note(wt, MSG_INFO, "Too short Deauthentication frame from " MACSTR, MAC2STR(mgmt->sa));
		return;
	}

	reason = le_to_host16(mgmt->u.deauth.reason_code);
	wpa_printf(MSG_DEBUG, "DEAUTH " MACSTR " -> " MACSTR " (reason=%u) (valid=%d)",
	...
	if (os_memcmp(mgmt->sa, mgmt->bssid, ETH_ALEN) == 0)
	{
		sta->counters[valid ? WLANTEST_STA_COUNTER_VALID_DEAUTH_RX : WLANTEST_STA_COUNTER_INVALID_DEAUTH_RX]++;
		...
		sta->counters[valid ? WLANTEST_STA_COUNTER_VALID_DEAUTH_TX : WLANTEST_STA_COUNTER_INVALID_DEAUTH_TX]++;
							 ...
							 const u8 *encr_end)
{
	...
	wpa_printf(MSG_DEBUG, "Trying to derive PTK for " MACSTR " from FILS rMSK", MAC2STR(sta->addr));
...
static void rx_mgmt_assoc_req(struct wlantest *wt, const u8 *data, size_t len)
{
	...
	wpa_printf(MSG_DEBUG, "ASSOCREQ " MACSTR " -> " MACSTR " (capab=0x%x listen_int=%u)",
			   MAC2STR(mgmt->sa), MAC2STR(mgmt->da),
...
static void rx_mgmt_assoc_resp(struct wlantest *wt, const u8 *data, size_t len)
{
	...
	wpa_printf(MSG_DEBUG, "ASSOCRESP " MACSTR " -> " MACSTR " (capab=0x%x status=%u aid=%u)",
	...
	if (status == WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY)
	{
		...
		{
			sta->counters[WLANTEST_STA_COUNTER_ASSOCRESP_COMEBACK]++;
		}
	}
	...
	if (sta->state < STATE3)
	{
		add_note(wt, MSG_DEBUG, "STA " MACSTR " moved to State 3 with " MACSTR,
				 MAC2STR(sta->addr), MAC2STR(bss->bssid));
		sta->state = STATE3;
	}
								...
								size_t len)
{
	...
	wpa_printf(MSG_DEBUG, "REASSOCREQ " MACSTR " -> " MACSTR " (capab=0x%x listen_int=%u current_ap=" MACSTR ")",
			   MAC2STR(mgmt->sa), MAC2STR(mgmt->da),
								 ...
								 size_t len)
{
	...
	wpa_printf(MSG_DEBUG, "REASSOCRESP " MACSTR " -> " MACSTR " (capab=0x%x status=%u aid=%u)",
			   MAC2STR(mgmt->sa), MAC2STR(mgmt->da), capab, status,
			   aid & 0x3fff);
	...
	if (sta->auth_alg == WLAN_AUTH_FILS_SK)
	{
		...
		if (session)
		{
			frame_ad = (const u8 *)&mgmt->u.reassoc_resp.capab_info;
			frame_ad_end = session + 2 + session[1];
			encr_end = data + len;
			decrypt_fils_assoc_resp(wt, bss, sta, data, frame_ad,
									frame_ad_end, encr_end);
	...
	if (status == WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY)
	{
		...
		{
			sta->counters[WLANTEST_STA_COUNTER_REASSOCRESP_COMEBACK]++;
		}
	}
							 ...
							 int valid)
{
	...
	if (len < 24 + 2)
	{
		add_note(wt, MSG_INFO, "Too short Disassociation frame from " MACSTR, MAC2STR(mgmt->sa));
		return;
	}

	reason = le_to_host16(mgmt->u.disassoc.reason_code);
	wpa_printf(MSG_DEBUG, "DISASSOC " MACSTR " -> " MACSTR " (reason=%u) (valid=%d)",
			   MAC2STR(mgmt->sa), MAC2STR(mgmt->da),
			   reason, valid);
	...
	if (os_memcmp(mgmt->sa, mgmt->bssid, ETH_ALEN) == 0)
	{
		sta->counters[valid ? WLANTEST_STA_COUNTER_VALID_DISASSOC_RX : WLANTEST_STA_COUNTER_INVALID_DISASSOC_RX]++;
		if (sta->pwrmgt && !sta->pspoll)
			sta->counters[WLANTEST_STA_COUNTER_DISASSOC_RX_ASLEEP]++;
		else
			sta->counters[WLANTEST_STA_COUNTER_DISASSOC_RX_AWAKE]++;
		...
		sta->counters[valid ? WLANTEST_STA_COUNTER_VALID_DISASSOC_TX : WLANTEST_STA_COUNTER_INVALID_DISASSOC_TX]++;
										...
										size_t len, int valid)
{
	...
	add_note(wt, MSG_INFO, "SA Query Request " MACSTR " -> " MACSTR " (trans_id=%02x%02x)%s",
	...
	if (os_memcmp(mgmt->sa, sta->addr, ETH_ALEN) == 0)
		sta->counters[valid ? WLANTEST_STA_COUNTER_VALID_SAQUERYREQ_TX : WLANTEST_STA_COUNTER_INVALID_SAQUERYREQ_TX]++;
	else
		sta->counters[valid ? WLANTEST_STA_COUNTER_VALID_SAQUERYREQ_RX : WLANTEST_STA_COUNTER_INVALID_SAQUERYREQ_RX]++;
}
										 ...
										 size_t len, int valid)
{
	...
	add_note(wt, MSG_INFO, "SA Query Response " MACSTR " -> " MACSTR " (trans_id=%02x%02x; %s)%s",
			 MAC2STR(mgmt->sa), MAC2STR(mgmt->da), rx_id[0], rx_id[1],
			 match ? "match" : "mismatch",
			 valid ? "" : " (invalid protection)");
	if (os_memcmp(mgmt->sa, sta->addr, ETH_ALEN) == 0)
		sta->counters[(valid && match) ? WLANTEST_STA_COUNTER_VALID_SAQUERYRESP_TX : WLANTEST_STA_COUNTER_INVALID_SAQUERYRESP_TX]++;
	else
		sta->counters[(valid && match) ? WLANTEST_STA_COUNTER_VALID_SAQUERYRESP_RX : WLANTEST_STA_COUNTER_INVALID_SAQUERYRESP_RX]++;
}
						   ...
						   int valid)
{
	...
	if (mgmt->da[0] & 0x01)
	{
		add_note(wt, MSG_DEBUG, "Group addressed Action frame: DA=" MACSTR " SA=" MACSTR " BSSID=" MACSTR " category=%u",
	...
	wpa_printf(MSG_DEBUG, "ACTION " MACSTR " -> " MACSTR " (category=%u) (valid=%d)",
			   MAC2STR(mgmt->sa), MAC2STR(mgmt->da),
...
static int check_bip(struct wlantest *wt, const u8 *data, size_t len)
{
		...
		data[len - (10 + mic_len - 1)] != 8 + mic_len)
	{
		...
		if (bss->rsn_capab & WPA_CAPABILITY_MFPC)
		{
			add_note(wt, MSG_INFO, "Robust group-addressed "
								   "management frame sent without BIP by " MACSTR,
					 MAC2STR(mgmt->sa));
	...
	{
		add_note(wt, MSG_INFO, "Invalid MMIE MIC in a frame from " MACSTR, MAC2STR(mgmt->sa));
		bss->counters[WLANTEST_BSS_COUNTER_INVALID_BIP_MMIE]++;
		return -1;
	}
							 ...
							 size_t *dlen)
{
	...
	if (!(data[24 + 3] & 0x20))
	{
		add_note(wt, MSG_INFO, "Expected CCMP frame from " MACSTR " did not have ExtIV bit set to 1",
				 MAC2STR(hdr->addr2));
		return NULL;
	}
	...
	if (keyid != 0)
	{
		add_note(wt, MSG_INFO, "Unexpected non-zero KeyID %d in "
							   "individually addressed Management frame from " MACSTR,
	...
	if (os_memcmp(pn, rsc, 6) <= 0)
	{
		...
		add_note(wt, MSG_INFO, "CCMP/TKIP replay detected: A1=" MACSTR " A2=" MACSTR " A3=" MACSTR " seq=%u frag=%u%s",
				 MAC2STR(hdr->addr1), MAC2STR(hdr->addr2),
				 MAC2STR(hdr->addr3),
				 WLAN_GET_SEQ_SEQ(seq_ctrl),
				 WLAN_GET_SEQ_FRAG(seq_ctrl),
				 (le_to_host16(hdr->frame_control) & WLAN_FC_RETRY) ? " Retry" : "");
...
static int check_mgmt_ccmp(struct wlantest *wt, const u8 *data, size_t len)
{
		 ...
		 WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_ACTION))
	{
		add_note(wt, MSG_INFO, "Robust individually-addressed "
							   "management frame sent without CCMP by " MACSTR,
				 MAC2STR(mgmt->sa));
		return -1;
	}
							   ...
							   const struct ieee80211_hdr *hdr)
{
	...
	add_note(wt, MSG_DEBUG, "DEAUTH from " MACSTR " acknowledged by " MACSTR, MAC2STR(mgmt->sa), MAC2STR(mgmt->da));
	if (os_memcmp(mgmt->sa, mgmt->bssid, ETH_ALEN) == 0)
	{
		int c;
		c = wt->last_mgmt_valid ? WLANTEST_STA_COUNTER_VALID_DEAUTH_RX_ACK : WLANTEST_STA_COUNTER_INVALID_DEAUTH_RX_ACK;
		sta->counters[c]++;
	}
}
								 ...
								 const struct ieee80211_hdr *hdr)
{
	...
	add_note(wt, MSG_DEBUG, "DISASSOC from " MACSTR " acknowledged by " MACSTR, MAC2STR(mgmt->sa), MAC2STR(mgmt->da));
	if (os_memcmp(mgmt->sa, mgmt->bssid, ETH_ALEN) == 0)
	{
		int c;
		c = wt->last_mgmt_valid ? WLANTEST_STA_COUNTER_VALID_DISASSOC_RX_ACK : WLANTEST_STA_COUNTER_INVALID_DISASSOC_RX_ACK;
		sta->counters[c]++;
	}
}
...
void rx_mgmt_ack(struct wlantest *wt, const struct ieee80211_hdr *hdr)
{
	...
	wpa_printf(MSG_MSGDUMP, "MGMT ACK: stype=%u a1=" MACSTR " a2=" MACSTR " a3=" MACSTR,
...
void rx_ext(struct wlantest *wt, const u8 *data, size_t len)
{
	if (len < 24)
		return;

	wt->rx_ext++;
	rx_mgmt_s1g_beacon(wt, data, len);
}
