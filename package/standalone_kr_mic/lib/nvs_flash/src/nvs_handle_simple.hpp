...
public:
    ...
    nvs_err_t set_typed_item(ItemType datatype, const char *key, const void *data, size_t dataSize) override;

    nvs_err_t get_typed_item(ItemType datatype, const char *key, void *data, size_t dataSize) override;

    nvs_err_t set_string(const char *key, const char *str) override;

    nvs_err_t set_blob(const char *key, const void *blob, size_t len) override;

    nvs_err_t get_string(const char *key, char *out_str, size_t len) override;

    nvs_err_t get_blob(const char *key, void *out_blob, size_t len) override;

    nvs_err_t get_item_size(ItemType datatype, const char *key, size_t &size) override;

    nvs_err_t erase_item(const char *key) override;

    nvs_err_t erase_all() override;

    nvs_err_t commit() override;

    nvs_err_t get_used_entry_count(size_t &usedEntries) override;

    nvs_err_t getItemDataSize(ItemType datatype, const char *key, size_t &dataSize);

    void debugDump();

    nvs_err_t fillStats(nvs_stats_t &nvsStats);

    nvs_err_t calcEntriesInNamespace(size_t &usedEntries);
