...
#ifdef REG_DEBUG_ON
    if (ret < 0) {
        system_printf("READ REG 0x%04x FAILED: %d\n", reg, ret);
    }
#endif
    return ret;
}
...
static int write_reg(uint8_t slv_addr, const uint16_t reg, uint8_t value){
#ifndef REG_DEBUG_ON
    SCCB_Write16(slv_addr, reg, value);
#else
    ...
    if ((uint8_t)old_value != value) {
        system_printf("NEW REG 0x%04x: 0x%02x to 0x%02x\n", reg, (uint8_t)old_value, value);
        SCCB_Write16(slv_addr, reg, value);
    } else {
        system_printf("OLD REG 0x%04x: 0x%02x\n", reg, (uint8_t)old_value);
        SCCB_Write16(slv_addr, reg, value);//maybe not?
    }
#endif
    return 0;
}
...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div, int pre_div, bool root_2x, int pclk_root_div, bool pclk_manual, int pclk_div)
{
	...
	system_printf("xclk : %d, pll_bypass : %s, pll_multiplier : %d, pll_sys_div : %d, pre_div : %d, root_2x : %s, pclk_root_div : %d, pclk_manual : %s, pclk_div : %d\n",
				  xclk, pll_bypass?"true":"false", pll_multiplier, pll_sys_div, pre_div, root_2x?"true":"false", pclk_root_div, pclk_manual?"true":"false", pclk_div);
	system_printf("pll_pre_div : %.2f, root_2x_div : %d, pll_pclk_root_div : %d\n", pll_pre_div, root_2x_div, pll_pclk_root_div);
    ...
    system_printf("Calculated XVCLK: %d Hz, REFIN: %u Hz, VCO: %u Hz, PLL_CLK: %u Hz, SYSCLK: %u Hz, PCLK: %u Hz\n", xclk, REFIN, VCO, PLL_CLK, SYSCLK, PCLK);
    return SYSCLK;
}
...
static int set_pll(sensor_t *sensor, bool bypass, uint8_t multiplier, uint8_t sys_div, uint8_t pre_div, bool root_2x, uint8_t pclk_root_div, bool pclk_manual, uint8_t pclk_div){
    ...
    if(multiplier > 252 || multiplier < 4 || sys_div > 15 || pre_div > 8 || pclk_div > 31 || pclk_root_div > 3){
        system_printf("Invalid arguments\n");
        return -1;
    }
    if(multiplier > 127){
        multiplier &= 0xFE;//only even integers above 127
    }
    system_printf("Set PLL: bypass: %u, multiplier: %u, sys_div: %u, pre_div: %u, root_2x: %u, pclk_root_div: %u, pclk_manual: %u, pclk_div: %u\n", bypass, multiplier, sys_div, pre_div, root_2x, pclk_root_div, pclk_manual, pclk_div);
    ...
    if(ret){
        system_printf("set_sensor_pll FAILED!\n");
    }
    return ret;
}
...
static int reset(sensor_t *sensor)
{
    ...
    if(ret){
        system_printf("Software Reset FAILED!\n");
        return ret;
    }
    vTaskDelay(100 / portTICK_PERIOD_MS);
    ret = write_regs(sensor->slv_addr, sensor_default_regs);
    if (ret == 0) {
        system_printf("Camera defaults loaded\n");
        vTaskDelay(100 / portTICK_PERIOD_MS);
...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
    ...
    default:
        system_printf("Unsupported pixformat: %u\n", pixformat);
        return -1;
    }
    ...
    if(ret == 0) {
        ...
        system_printf("Set pixformat to: %u\n", pixformat);
    }
    return ret;
}
...
static int set_image_options(sensor_t *sensor)
{
    ...
    if(write_reg(sensor->slv_addr, TIMING_TC_REG20, reg20)
        ...
        system_printf("Setting Image Options Failed\n");
        return -1;
    }
    ...
    system_printf("Set Image Options: Compression: %u, Binning: %u, V-Flip: %u, H-Mirror: %u, Reg-4514: 0x%02x\n",
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    if(framesize > FRAMESIZE_QSXGA){
        system_printf("Invalid framesize: %u\n", framesize);
        return -1;
    }
    ...
    if (sensor->pixformat == PIXFORMAT_JPEG) {
        ...
        /* sysclk = xvclk / sys_div  * 10 */
        /* 10Mhz (sysclk) / pclk_div  = pclk*/
        /* pclk_div : 15 : pclk : 666666 Hz */
        /* pclk_div : 16 : pclk : 625000 Hz */
        /* On nrc7292 pclk should be lower than 625000 Hz to correctly retrieve image data. */
        //ret = set_pll(sensor, false, sys_mul, 4, 2, false, 2, true, 4);
        //ret = set_pll(sensor, false, sys_mul, 8, 2, false, 2, true, 10);
        ret = set_pll(sensor, false, sys_mul, 8, 2, false, 2, true, 0x8);
    ...
    if (ret == 0) {
        system_printf("Set framesize to: %ux%u\n", w, h);
    }
    return ret;

fail:
    sensor->status.framesize = old_framesize;
    system_printf("Setting framesize to: %ux%u failed\n", w, h);
    return ret;
}
...
static int set_hmirror(sensor_t *sensor, int enable)
{
    ...
    ret = set_image_options(sensor);
    if (ret == 0) {
        system_printf("Set h-mirror to: %d\n", enable);
    }
    return ret;
}
...
static int set_quality(sensor_t *sensor, int qs)
{
    ...
    if (ret == 0) {
        ...
        system_printf("Set quality to: %d\n", qs);
    }
    return ret;
}
...
static int set_colorbar(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0) {
        ...
        system_printf("Set colorbar to: %d\n", enable);
    }
    return ret;
}
...
static int set_gain_ctrl(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0) {
        system_printf("Set gain_ctrl to: %d\n", enable);
...
static int set_exposure_ctrl(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0) {
        system_printf("Set exposure_ctrl to: %d\n", enable);
...
static int set_raw_gma_dsp(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0) {
        system_printf("Set raw_gma to: %d\n", enable);
...
static int set_agc_gain(sensor_t *sensor, int gain)
{
    ...
    if (ret == 0) {
        system_printf("Set agc_gain to: %d\n", gain);
...
static int set_aec_value(sensor_t *sensor, int value)
{
    ...
    if (max_val < 0) {
        system_printf("Could not read max aec_value\n");
        return -1;
    }
    if (value > max_val) {
        value =max_val;
    ...
    if (ret == 0) {
        system_printf("Set aec_value to: %d / %d\n", value, max_val);
...
static int set_ae_level(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        system_printf("Set ae_level to: %d\n", level);
...
static int set_wb_mode(sensor_t *sensor, int mode)
{
    ...
    if (ret == 0) {
        system_printf("Set wb_mode to: %d\n", mode);
...
static int set_awb_gain_dsp(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0) {
        ...
        system_printf("Set awb_gain to: %d\n", enable);
...
static int set_special_effect(sensor_t *sensor, int effect)
{
    ...
    if (ret == 0) {
        system_printf("Set special_effect to: %d\n", effect);
...
static int set_brightness(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        system_printf("Set brightness to: %d\n", level);
...
static int set_contrast(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        system_printf("Set contrast to: %d\n", level);
...
static int set_saturation(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        system_printf("Set saturation to: %d\n", level);
...
static int set_sharpness(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        system_printf("Set sharpness to: %d\n", level);
...
static int set_gainceiling(sensor_t *sensor, gainceiling_t level)
{
    ...
    if (ret == 0) {
        system_printf("Set gainceiling to: %d\n", l);
...
static int set_denoise(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        system_printf("Set denoise to: %d\n", level);
...
int ov5640_detect(int slv_addr, sensor_id_t *id)
{
    if (OV5640_SCCB_ADDR == slv_addr) {
        ...
        if (OV5640_PID == PID) {
            ...
            system_printf("Mismatch h=0x%x, l=0x%x, PID=0x%x\n", h, l, PID);
        }
    }
    return 0;
}
...
int cmd_test_cam_write(cmd_tbl_t *t, int argc, char *argv[])
{

	uint32_t addr = strtoul(argv[1], NULL, 0);
	uint32_t val = strtoul(argv[2], NULL, 0);
	int ret = 0;
    ret = write_reg(OV5640_SCCB_ADDR, addr, val);
	if (ret == 0)
...
SUBCMD_MAND(test,
	...
	cmd_test_cam_write,
	"OV5640 write reg",
	"test cam_write [addr] [value]");

int cmd_test_cam_read(cmd_tbl_t *t, int argc, char *argv[])
{
	uint16_t addr = (uint16_t) strtoul(argv[1], NULL, 0);
	int ret = 0;
    ret = read_reg(OV5640_SCCB_ADDR, addr);
	system_printf("Addr:0x%X - 0x%X\n", addr, ret);
