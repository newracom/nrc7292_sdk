...
coap_new_context(
  ...
  c->csm_timeout = 30;  // CSM (Capabilities and Settings Messages)
...
#if HAVE_MBEDTLS
    else if (coap_dtls_is_handshake_established(session))
...
coap_read_session(coap_context_t *ctx, coap_session_t *session, coap_tick_t now) {
  ...
  if (COAP_PROTO_NOT_RELIABLE(session->proto)) {
    ...
    if(session->proto ==  COAP_PROTO_DTLS){
        bytes_read = coap_dtls_read(session, packet);
    }
    else
#endif
...
#if defined(HAVE_MBEDTLS)
    if(coap_dtls_is_handshake_established(endpoint->sessions)){
        bytes_read = coap_dtls_read(endpoint->sessions, packet);
    }
    else
#endif
...
#if HAVE_MBEDTLS
    if(bytes_read == -2){
      coap_session_free(endpoint->sessions);
      return 0;
    }
#else
