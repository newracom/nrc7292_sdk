...
void setup_wifi_ap_mode2(void)
{
	A("%s\n", __func__);
	wifi_set_opmode((wifi_get_opmode()|WIFI_STATIONAP_MODE)&USE_WIFI_MODE);
	wifi_get_ip_info(SOFTAP_IF, &ipinfo[SOFTAP_IF]);

	A("after => ip:%s\n",
		ip4addr_ntoa((const ip4_addr_t*)ip_2_ip4(&ipinfo[SOFTAP_IF].ip)));
	A("after => gw:%s\n",
		ip4addr_ntoa((const ip4_addr_t*)ip_2_ip4(&ipinfo[SOFTAP_IF].gw)));
	A("after => netmask:%s\n",
		ip4addr_ntoa((const ip4_addr_t*)ip_2_ip4(&ipinfo[SOFTAP_IF].netmask)));

	wifi_set_ip_info(SOFTAP_IF, &ipinfo[SOFTAP_IF]);
...
#if LWIP_DHCPS
	dhcps_start(&ipinfo[SOFTAP_IF], SOFTAP_IF);
#endif
}
#endif
...
void setup_wifi_ap_mode(int vif)
{
	...
	wifi_set_opmode((wifi_get_opmode()|WIFI_STATIONAP_MODE)&USE_WIFI_MODE);
	memset((void*)&apconfig.ssid, 0, sizeof(apconfig.ssid));
	memset((void*)&apconfig.password, 0, sizeof(apconfig.password));
	apconfig.ssid_len = sprintf((void*)&apconfig.ssid, WIFI_AP_NAME);
	sprintf((void*)&apconfig.password, "%s", WIFI_AP_PASSWORD);
