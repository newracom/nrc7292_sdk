...
int httpd_send(httpd_req_t *r, const char *buf, size_t buf_len)
{
    ...
    if (ret < 0) {
        V(TAG, LOG_FMT("error in send_fn"));
        return ret;
    }
    return ret;
}
...
{
    V(TAG, LOG_FMT("requested length = %d"), buf_len);
    ...
    if (ra->sd->pending_len > 0) {
        V(TAG, LOG_FMT("pending length = %d"), ra->sd->pending_len);
    ...
    if (ret < 0) {
        V(TAG, LOG_FMT("error in recv_fn"));
    ...
    V(TAG, LOG_FMT("received length = %d"), ret + pending_len);
...
size_t httpd_unrecv(struct httpd_req *r, const char *buf, size_t buf_len)
{
    ...
    V(TAG, LOG_FMT("length = %d"), ra->sd->pending_len);
...
esp_err_t httpd_resp_set_hdr(httpd_req_t *r, const char *field, const char *value)
{
    ...
    V(TAG, LOG_FMT("new header = %s: %s"), field, value);
    return ESP_OK;
}
...
esp_err_t httpd_resp_send_err(httpd_req_t *req, httpd_err_code_t error, const char *usr_msg)
{
    ...
    if (setsockopt(ra->sd->fd, IPPROTO_TCP, TCP_NODELAY, &nodelay, sizeof(nodelay)) < 0) {
        ...
        E(TAG, LOG_FMT("error calling setsockopt : %d"), errno);
        nodelay = 0;
    }
#endif
...
int httpd_req_recv(httpd_req_t *r, char *buf, size_t buf_len)
{
    ...
    if (!httpd_valid_req(r)) {
        E(TAG, LOG_FMT("invalid request"));
        return HTTPD_SOCK_ERR_INVALID;
    }

    struct httpd_req_aux *ra = r->aux;
    V(TAG, LOG_FMT("remaining length = %d"), ra->remaining_len);
    ...
    V(TAG, LOG_FMT("received length = %d"), ret);
...
{
    int errval;
    E(TAG, LOG_FMT("error in %s : %d"), ctx, errno);
