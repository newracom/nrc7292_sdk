...
static int enum_function(struct sock_db *session, void *context)
{
    ...
    case HTTPD_TASK_DELETE_INVALID:
        if (!fd_is_valid(session->fd)) {
            E(TAG, LOG_FMT("Closing invalid socket %d"), session->fd);
    ...
    case HTTPD_TASK_CLOSE:
        if (session->fd != -1) {
            V(TAG, LOG_FMT("cleaning up socket %d"), session->fd);
            httpd_sess_delete(ctx->hd, session);
        }
        break;
    default:
...
static void httpd_sess_close(void *arg)
{
    ...
    if (!sock_db->lru_counter && !sock_db->lru_socket) {
        V(TAG, "Skipping session close for %d as it seems to be a race condition", sock_db->fd);
        return;
    }
...
{
    V(TAG, LOG_FMT("fd = %d"), newfd);

    if (httpd_sess_get(hd, newfd)) {
        E(TAG, LOG_FMT("session already exists with fd = %d"), newfd);
        return ESP_FAIL;
    }

    struct sock_db *session = httpd_sess_get_free(hd);
    if (!session) {
        V(TAG, LOG_FMT("unable to launch session for fd = %d"), newfd);
        return ESP_FAIL;
    }
    ...
    if (hd->config.open_fn) {
        ...
        if (ret != ESP_OK) {
            ...
            V(TAG, LOG_FMT("open_fn failed for fd = %d"), newfd);
            return ret;
        }
    }


    V(TAG, LOG_FMT("active sockets: %d"), hd->hd_sd_active_count);
    return ESP_OK;
}
...
void httpd_sess_delete(struct httpd_data *hd, struct sock_db *session)
{
    ...
    if (hd->config.enable_so_linger) {
        ...
        if (setsockopt(session->fd, SOL_SOCKET, SO_LINGER, &so_linger, sizeof(struct linger)) < 0) {
            E(TAG, LOG_FMT("error enabling SO_LINGER (%d)"), errno);
        }
    }
...
esp_err_t httpd_sess_process(struct httpd_data *hd, struct sock_db *session)
{
    ...
    V(TAG, LOG_FMT("httpd_req_new"));
    if (httpd_req_new(hd, session) != ESP_OK) {
        return ESP_FAIL;
    }
    V(TAG, LOG_FMT("httpd_req_delete"));
    if (httpd_req_delete(hd) != ESP_OK) {
        return ESP_FAIL;
    }
    V(TAG, LOG_FMT("success"));
...
esp_err_t httpd_sess_close_lru(struct httpd_data *hd)
{
    ...
    V(TAG, LOG_FMT("Closing session with fd %d"), context.session->fd);
