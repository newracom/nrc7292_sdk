...
#define HTTPD_MAX_SOCKETS MEMP_NUM_TCP_PCB
...
static esp_err_t httpd_accept_conn(struct httpd_data *hd, int listen_fd)
{
    ...
    if (new_fd < 0) {
        E(TAG, LOG_FMT("error in accept (%d)"), errno);
        return ESP_FAIL;
    }
    V(TAG, LOG_FMT("newfd = %d"), new_fd);
    ...
    if (setsockopt(new_fd, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv, sizeof(tv)) < 0) {
        E(TAG, LOG_FMT("error in setsockopt SO_RCVTIMEO (%d)"), errno);
        goto exit;
    }
    ...
    if (setsockopt(new_fd, SOL_SOCKET, SO_SNDTIMEO, (const char *)&tv, sizeof(tv)) < 0) {
        E(TAG, LOG_FMT("error in setsockopt SO_SNDTIMEO (%d)"), errno);
        goto exit;
    }
...
esp_err_t httpd_queue_work(httpd_handle_t handle, httpd_work_fn_t work, void *arg)
{
    ...
    E(TAG, "Unable to acquire semaphore");
    return ESP_FAIL;
#endif
}
...
static void httpd_process_ctrl_msg(struct httpd_data *hd)
{
    ...
    if (ret <= 0) {
        E(TAG, LOG_FMT("error in recv (%d)"), errno);
#if CONFIG_HTTPD_QUEUE_WORK_BLOCKING
        xSemaphoreGive(hd->ctrl_sock_semaphore);
#endif
        return;
...
static int httpd_process_session(struct sock_db *session, void *context)
{
    ...
    if (FD_ISSET(fd, ctx->fdset) || httpd_sess_pending(ctx->hd, session)) {
        V(TAG, LOG_FMT("processing socket %d"), fd);
...
static esp_err_t httpd_server(struct httpd_data *hd)
{
    ...
    V(TAG, LOG_FMT("doing select maxfd+1 = %d"), maxfd + 1);
    int active_cnt = select(maxfd + 1, &read_set, NULL, NULL, NULL);
    if (active_cnt < 0) {
        E(TAG, LOG_FMT("error in select (%d)"), errno);
    ...
    if (FD_ISSET(hd->ctrl_fd, &read_set)) {
        V(TAG, LOG_FMT("processing ctrl message"));
        httpd_process_ctrl_msg(hd);
        if (hd->hd_td.status == THREAD_STOPPING) {
            V(TAG, LOG_FMT("stopping thread"));
            return ESP_FAIL;
        }
    }
    ...
    if (FD_ISSET(hd->listen_fd, &read_set)) {
        V(TAG, LOG_FMT("processing listen socket %d"), hd->listen_fd);
        if (httpd_accept_conn(hd, hd->listen_fd) != ESP_OK) {
            E(TAG, LOG_FMT("error accepting new connection"));
        }
    }
    return ESP_OK;
}
...
static void httpd_thread(void *arg)
{
    ...
    V(TAG, LOG_FMT("web server started"));
    while (1) {
        ret = httpd_server(hd);
        if (ret != ESP_OK) {
            break;
    ...
    V(TAG, LOG_FMT("web server exiting"));
    close(hd->msg_fd);
...
static esp_err_t httpd_server_init(struct httpd_data *hd)
{
...
#endif
    if (fd < 0) {
        E(TAG, LOG_FMT("error in socket (%d)"), errno);
        return ESP_FAIL;
    }
#if CONFIG_LWIP_IPV6
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(enable)) < 0) {
        ...
        E(TAG, LOG_FMT("error in setsockopt SO_REUSEADDR (%d)"), errno);
    }

    int ret = bind(fd, (struct sockaddr *)&serv_addr, sizeof(serv_addr));
    if (ret < 0) {
        E(TAG, LOG_FMT("error in bind (%d)"), errno);
        close(fd);
        return ESP_FAIL;
    }
    ...
    if (ret < 0) {
        E(TAG, LOG_FMT("error in listen (%d)"), errno);
        close(fd);
        return ESP_FAIL;
    }
    ...
    if (ctrl_fd < 0) {
        E(TAG, LOG_FMT("error in creating ctrl socket (%d)"), errno);
        close(fd);
        return ESP_FAIL;
    }
    ...
    if (msg_fd < 0) {
        E(TAG, LOG_FMT("error in creating msg socket (%d)"), errno);
        close(fd);
        close(ctrl_fd);
        return ESP_FAIL;
    }
...
static struct httpd_data *httpd_create(const httpd_config_t *config)
{
    ...
    if (!hd) {
        E(TAG, LOG_FMT("Failed to allocate memory for HTTP server instance"));
        return NULL;
    }
    hd->hd_calls = calloc(config->max_uri_handlers, sizeof(httpd_uri_t *));
    if (!hd->hd_calls) {
        E(TAG, LOG_FMT("Failed to allocate memory for HTTP URI handlers"));
        free(hd);
        return NULL;
    }
    hd->hd_sd = calloc(config->max_open_sockets, sizeof(struct sock_db));
    if (!hd->hd_sd) {
        E(TAG, LOG_FMT("Failed to allocate memory for HTTP session data"));
        free(hd->hd_calls);
        free(hd);
        return NULL;
    }
    ...
    if (!ra->resp_hdrs) {
        E(TAG, LOG_FMT("Failed to allocate memory for HTTP response headers"));
        free(hd->hd_sd);
        free(hd->hd_calls);
        free(hd);
        return NULL;
    ...
    if (!hd->err_handler_fns) {
        E(TAG, LOG_FMT("Failed to allocate memory for HTTP error handlers"));
        free(ra->resp_hdrs);
        free(hd->hd_sd);
        free(hd->hd_calls);
        free(hd);
...
esp_err_t httpd_start(httpd_handle_t *handle, const httpd_config_t *config)
{
    ...
    if (HTTPD_MAX_SOCKETS < config->max_open_sockets + 3) {
        E(TAG, "Config option max_open_sockets is too large (max allowed %d, 3 sockets used by HTTP server internally)\n\t"
    ...
    if (hd->ctrl_sock_semaphore == NULL) {
        E(TAG, "Failed to create Semaphore");
        httpd_delete(hd);
        return ESP_ERR_HTTPD_ALLOC_MEM;
    }
#endif
...
esp_err_t httpd_stop(httpd_handle_t handle)
{
    ...
    if ((ret = cs_send_to_ctrl_sock(hd->msg_fd, hd->config.ctrl_port, &msg, sizeof(msg))) < 0) {
        E(TAG, "Failed to send shutdown signal err=%d", ret);
        return ESP_FAIL;
    }

    V(TAG, LOG_FMT("sent control msg to stop server"));
