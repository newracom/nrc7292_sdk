...
static bool httpd_ws_get_response_subprotocol(const char *supported_subprotocol, char *subprotocol, size_t buf_len)
{
    ...
    if (supported_subprotocol == NULL) {
        E(TAG, "Sec-WebSocket-Protocol %s not supported, URI do not support any subprotocols", subprotocol);
        return false;
    }
    ...
    do {
        if (strncmp(s, supported_subprotocol, sizeof(subprotocol)) == 0) {
            V(TAG, "Requested subprotocol supported: %s", s);
            return true;
        }
    } while ((s = strtok_r(NULL, ", ", &rest)) != NULL);

    E(TAG, "Sec-WebSocket-Protocol %s not supported, supported subprotocol is %s", subprotocol, supported_subprotocol);
...
esp_err_t httpd_ws_respond_server_handshake(httpd_req_t *req, const char *supported_subprotocol)
{
    ...
    if (!req || !req->aux) {
        E(TAG, LOG_FMT("Argument is invalid"));
    ...
    if (req_aux->sd->ws_handshake_done) {
        E(TAG, LOG_FMT("State is invalid - Handshake has been performed"));
    ...
    if (httpd_req_get_hdr_value_str(req, "Sec-WebSocket-Version", version_val, sizeof(version_val)) != ESP_OK) {
        E(TAG, LOG_FMT("\"Sec-WebSocket-Version\" is not found"));
    ...
    if (strcasecmp(version_val, "13") != 0) {
        E(TAG, LOG_FMT("\"Sec-WebSocket-Version\" is not \"13\", it is: %s"), version_val);
    ...
    if (httpd_req_get_hdr_value_str(req, "Sec-WebSocket-Key", sec_key_encoded, sizeof(sec_key_encoded)) != ESP_OK) {
        E(TAG, LOG_FMT("Cannot find client key"));
        return ESP_ERR_NOT_FOUND;
    }
    ...
    V(TAG, LOG_FMT("Server key before encoding: %s"), server_raw_text);
    ...
    V(TAG, LOG_FMT("Generated server key: %s"), server_key_encoded);
    ...
    if (httpd_req_get_hdr_value_str(req, "Sec-WebSocket-Protocol", subprotocol, sizeof(subprotocol) - 1) == ESP_ERR_HTTPD_RESULT_TRUNC) {
        E(TAG, "Sec-WebSocket-Protocol length exceeded buffer size of %d, was trunctated", sizeof(subprotocol));
    }
    ...
    if (httpd_send(req, tx_buf, fmt_len) < 0) {
        E(TAG, LOG_FMT("Failed to send the response"));
        return ESP_FAIL;
    }
...
static esp_err_t httpd_ws_check_req(httpd_req_t *req)
{
    /* Probe if input parameters are valid or not */
    if (!req || !req->aux) {
        E(TAG, LOG_FMT("Argument is null"));
        return ESP_ERR_INVALID_ARG;
    }
    ...
    struct httpd_req_aux *req_aux = req->aux;
    if (!req_aux->sd->ws_handshake_done) {
        E(TAG, LOG_FMT("State is invalid - No handshake performed"));
        return ESP_ERR_INVALID_STATE;
    }
...
{
    if (len < 1 || !payload) {
        E(TAG, LOG_FMT("Invalid payload provided"));
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t httpd_ws_recv_frame(httpd_req_t *req, httpd_ws_frame_t *frame, size_t max_len)
{
    ...
    if (aux == NULL) {
        E(TAG, LOG_FMT("Invalid Aux pointer"));
        return ESP_ERR_INVALID_ARG;
    }

    if (!frame) {
        E(TAG, LOG_FMT("Frame pointer is invalid"));
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (frame->len == 0) {
        ...
        if (httpd_recv_with_opt(req, (char *)&second_byte, sizeof(second_byte), false) <= 0) {
            E(TAG, LOG_FMT("Failed to receive the second byte"));
            return ESP_FAIL;
        }
        ...
        if (init_len < 126) {
            ...
            if (httpd_recv_with_opt(req, (char *)length_bytes, sizeof(length_bytes), false) <= 0) {
                E(TAG, LOG_FMT("Failed to receive 2 bytes length"));
                return ESP_FAIL;
            }
        ...
        if (masked) {
            if (httpd_recv_with_opt(req, (char *)aux->mask_key, sizeof(aux->mask_key), false) <= 0) {
                E(TAG, LOG_FMT("Failed to receive mask key"));
                return ESP_FAIL;
            }
        } else {
            /* If the WS frame from client to server is not masked, it should be rejected.
             * Please refer to RFC6455 Section 5.2 for more details. */
            E(TAG, LOG_FMT("WS frame is not properly masked."));
            return ESP_ERR_INVALID_STATE;
        }
    }
    ...
    if (frame->len > max_len) {
        if (max_len == 0) {
            V(TAG, "regard max_len == 0 is OK for user to get frame len");
            return ESP_OK;
        }
        E(TAG, LOG_FMT("WS Message too long"));
    ...
    if (frame->payload == NULL) {
        E(TAG, LOG_FMT("Payload buffer is null"));
        return ESP_FAIL;
    }
    ...
    while (left_len > 0) {
        ...
        if (read_len <= 0) {
            E(TAG, LOG_FMT("Failed to receive payload"));
            return ESP_FAIL;
        }
        offset += read_len;
        left_len -= read_len;

        V(TAG, "Frame length: %d, Bytes Read: %d", frame->len, offset);
    }
...
esp_err_t httpd_ws_send_frame_async(httpd_handle_t hd, int fd, httpd_ws_frame_t *frame)
{
    ...
    if (sess->send_fn(hd, fd, (const char *)header_buf, tx_len, 0) < 0) {
        E(TAG, LOG_FMT("Failed to send WS header"));
        return ESP_FAIL;
    }
    ...
    if(frame->len > 0 && frame->payload != NULL) {
        if (sess->send_fn(hd, fd, (const char *)frame->payload, frame->len, 0) < 0) {
            E(TAG, LOG_FMT("Failed to send WS payload"));
            return ESP_FAIL;
        }
    }
...
esp_err_t httpd_ws_get_frame_type(httpd_req_t *req)
{
    ...
    if (sd == NULL) {
        E(TAG, LOG_FMT("Invalid sd pointer"));
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (httpd_recv_with_opt(req, (char *)&first_byte, sizeof(first_byte), false) <= 0) {
        ...
        E(TAG, LOG_FMT("Failed to read header byte (socket FD invalid), closing socket now"));
        aux->ws_final = true;
        aux->ws_type = HTTPD_WS_TYPE_CLOSE;
        return ESP_OK;
    }

    V(TAG, LOG_FMT("First byte received: 0x%02X"), first_byte);
    ...
    if (!sd->ws_control_frames) {
        V(TAG, LOG_FMT("Handler not requests control frames"));

        /* Reply to PING. For PONG and CLOSE, it will be handled elsewhere. */
        if (aux->ws_type == HTTPD_WS_TYPE_PING) {
            V(TAG, LOG_FMT("Got a WS PING frame, Replying PONG..."));
            ...
            if (httpd_ws_recv_frame(req, &frame, 126) != ESP_OK) {
                V(TAG, LOG_FMT("Cannot receive the full PING frame"));
                return ESP_ERR_INVALID_STATE;
            }
            ...
            V(TAG, LOG_FMT("Got a WS CLOSE frame, Replying CLOSE..."));
            ...
            if (httpd_ws_recv_frame(req, &frame, 126) != ESP_OK) {
                V(TAG, LOG_FMT("Cannot receive the full CLOSE frame"));
                return ESP_ERR_INVALID_STATE;
            }
