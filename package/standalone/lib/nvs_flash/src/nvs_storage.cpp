...
nvs_err_t Storage::populateBlobIndices(TBlobIndexList& blobIdxList)
{
    for (auto it = mPageManager.begin(); it != mPageManager.end(); ++it) {
        ...
        while (p.findItem(Page::NS_ANY, ItemType::BLOB_IDX, nullptr, itemIndex, item) == NVS_OK) {
            BlobIndexNode* entry = new (std::nothrow) BlobIndexNode;

            if (!entry) return NVS_ERR_NO_MEM;
...
void Storage::eraseOrphanDataBlobs(TBlobIndexList& blobIdxList)
{
    for (auto it = mPageManager.begin(); it != mPageManager.end(); ++it) {
         ...
         */
        while (p.findItem(Page::NS_ANY, ItemType::BLOB_DATA, nullptr, itemIndex, item) == NVS_OK) {
...
nvs_err_t Storage::init(uint32_t baseSector, uint32_t sectorCount)
{
	NVS_LOGD(TAG, "[%s] mPageManager.load(0x%x, %d, %d)", __func__, mPartition, baseSector, sectorCount);
    auto err = mPageManager.load(mPartition, baseSector, sectorCount);
    if (err != NVS_OK) {
		NVS_LOGD(TAG, "[%s] error loading page...", __func__);
        mState = StorageState::INVALID;
        return err;
    }

    // load namespaces list
	NVS_LOGD(TAG, "[%s] load namespaces list...", __func__);
	clearNamespaces();
    ...
    for (auto it = mPageManager.begin(); it != mPageManager.end(); ++it) {
        ...
        Item item;
		uint32_t seqNumber;
		p.getSeqNumber(seqNumber);
		NVS_LOGD(TAG, "[%s] iterating pages seqNumber = %d...", __func__, seqNumber);
        while (p.findItem(Page::NS_INDEX, ItemType::U8, nullptr, itemIndex, item) == NVS_OK) {
            NamespaceEntry* entry = new (std::nothrow) NamespaceEntry;

            if (!entry) {
				NVS_LOGD(TAG, "[%s] entry %s...", __func__, entry->mName);
                mState = StorageState::INVALID;
				NVS_LOGD(TAG, "[%s] StorageState::INVALID, no memory...", __func__);
                return NVS_ERR_NO_MEM;
            }

            item.getKey(entry->mName, sizeof(entry->mName));
            item.getValue(entry->mIndex);
			NVS_LOGD(TAG, "[%s] name : %s, value index : %d", __func__, entry->mName, entry->mIndex);
	...
	NVS_LOGD(TAG, "[%s] StorageState::ACTIVE...", __func__);
    // Populate list of multi-page index entries.
    TBlobIndexList blobIdxList;
	NVS_LOGD(TAG, "[%s] populateBlobIndices...", __func__);
    err = populateBlobIndices(blobIdxList);
    if (err != NVS_OK) {
		NVS_LOGD(TAG, "[%s] populateBlobIndices failed, no mem...", __func__);
        mState = StorageState::INVALID;
        return NVS_ERR_NO_MEM;
    }

    // Remove the entries for which there is no parent multi-page index.
	NVS_LOGD(TAG, "[%s] erase Orphan Data Blobs...", __func__);
	eraseOrphanDataBlobs(blobIdxList);
	NVS_LOGD(TAG, "[%s] Purge the blob index list...", __func__);
...
nvs_err_t Storage::findItem(uint8_t nsIndex, ItemType datatype, const char* key, Page* &page, Item& item, uint8_t chunkIdx, VerOffset chunkStart)
{
...
nvs_err_t Storage::writeMultiPageBlob(uint8_t nsIndex, const char* key, const void* data, size_t dataSize, VerOffset chunkStart)
{
    ...
    do {
        ...
        if (chunkCount == 0U && ((tailroom < dataSize) || (tailroom == 0 && dataSize == 0)) && tailroom < Page::CHUNK_MAX_SIZE/10) {
            ...
            err = NVS_ERR_NVS_NOT_ENOUGH_SPACE;
            break;
        }
        ...
        chunkCount++;
        assert(err != NVS_ERR_NVS_PAGE_FULL);
        if (err != NVS_OK) {
            break;
        } else {
...
nvs_err_t Storage::writeItem(uint8_t nsIndex, ItemType datatype, const char* key, const void* data, size_t dataSize)
{
    ...
    if (datatype == ItemType::BLOB) {
        ...
        if (findPage) {
            ...
            if (cmpMultiPageBlob(nsIndex, key, data, dataSize) == NVS_OK) {
                return NVS_OK;
            }

            if (findPage->state() == Page::PageState::UNINITIALIZED ||
                    findPage->state() == Page::PageState::INVALID) {
                NVS_ERROR_CHECK(findItem(nsIndex, datatype, key, findPage, item));
            }
        ...
        if (findPage) {
            ...
            if (err == NVS_ERR_FLASH_OP_FAIL) {
                return NVS_ERR_NVS_REMOVE_FAILED;
            }
            if (err != NVS_OK) {
                return err;
            }
        ...
        // since it may invoke an erasure of flash.
        if (findPage != nullptr &&
                findPage->cmpItem(nsIndex, datatype, key, data, dataSize) == NVS_OK) {
            return NVS_OK;
        }
...
nvs_err_t Storage::createOrOpenNamespace(const char* nsName, bool canCreate, uint8_t& nsIndex)
{
    if (mState != StorageState::ACTIVE) {
		NVS_LOGD(TAG, "[%s] mState(%d) != StorageState::ACTIVE", __func__, mState);
        return NVS_ERR_NVS_NOT_INITIALIZED;
    }
	NVS_LOGD(TAG, "[%s] Looking for %s in mNamespaces (%d)", __func__, nsName, mNamespaces.size());
    auto it = std::find_if(mNamespaces.begin(), mNamespaces.end(), [=] (const NamespaceEntry& e) -> bool {
        NVS_LOGD(TAG, "[%s] nsName : %s, e.mName : %s", __func__, nsName, e.mName);
        return strncmp(nsName, e.mName, sizeof(e.mName) - 1) == 0;
    });
    if (it == std::end(mNamespaces)) {
		NVS_LOGD(TAG, "[%s] %s doesn't exist in mNamespaces", __func__, nsName);
        if (!canCreate) {
			NVS_LOGD(TAG, "[%s] !canCreate", __func__);
            return NVS_ERR_NVS_NOT_FOUND;
        }
        ...
        if (ns == 255) {
			NVS_LOGD(TAG, "[%s] NVS_ERR_NVS_NOT_ENOUGH_SPACE", __func__);
            return NVS_ERR_NVS_NOT_ENOUGH_SPACE;
        }

        NamespaceEntry* entry = new (std::nothrow) NamespaceEntry;
        if (!entry) {
			NVS_LOGD(TAG, "[%s] NamespaceEntry allocation failed...", __func__);
            return NVS_ERR_NO_MEM;
        }

        auto err = writeItem(Page::NS_INDEX, ItemType::U8, nsName, &ns, sizeof(ns));
        if (err != NVS_OK) {
			NVS_LOGD(TAG, "[%s] writeItem failed...", __func__);
            return err;
        }
...
nvs_err_t Storage::readMultiPageBlob(uint8_t nsIndex, const char* key, void* data, size_t dataSize)
{
    Item item;
    Page* findPage = nullptr;
...
nvs_err_t Storage::cmpMultiPageBlob(uint8_t nsIndex, const char* key, const void* data, size_t dataSize)
{
    Item item;
    Page* findPage = nullptr;
...
nvs_err_t Storage::readItem(uint8_t nsIndex, ItemType datatype, const char* key, void* data, size_t dataSize)
{
    if (mState != StorageState::ACTIVE) {
        return NVS_ERR_NVS_NOT_INITIALIZED;
    }
...
nvs_err_t Storage::eraseMultiPageBlob(uint8_t nsIndex, const char* key, VerOffset chunkStart)
{
    if (mState != StorageState::ACTIVE) {
        return NVS_ERR_NVS_NOT_INITIALIZED;
    }
    ...
    for (uint8_t chunkNum = 0; chunkNum < chunkCount; chunkNum++) {
        ...
        if (err != NVS_OK && err != NVS_ERR_NVS_NOT_FOUND) {
            return err;
        } else if (err == NVS_ERR_NVS_NOT_FOUND) {
...
nvs_err_t Storage::eraseItem(uint8_t nsIndex, ItemType datatype, const char* key)
{
    if (mState != StorageState::ACTIVE) {
        return NVS_ERR_NVS_NOT_INITIALIZED;
    }
...
nvs_err_t Storage::eraseNamespace(uint8_t nsIndex)
{
    if (mState != StorageState::ACTIVE) {
        return NVS_ERR_NVS_NOT_INITIALIZED;
    }
...
nvs_err_t Storage::getItemDataSize(uint8_t nsIndex, ItemType datatype, const char* key, size_t& dataSize)
{
    if (mState != StorageState::ACTIVE) {
        return NVS_ERR_NVS_NOT_INITIALIZED;
    }
...
void Storage::debugCheck()
{
    ...
    for (auto p = mPageManager.begin(); p != mPageManager.end(); ++p) {
        ...
        size_t usedCount = 0;
        Item item;
        while (p->findItem(Page::NS_ANY, ItemType::ANY, nullptr, itemIndex, item) == NVS_OK) {
...
nvs_err_t Storage::fillStats(nvs_stats_t& nvsStats)
{
    nvsStats.namespace_count = mNamespaces.size();
    return mPageManager.fillStats(nvsStats);
}

nvs_err_t Storage::calcEntriesInNamespace(uint8_t nsIndex, size_t& usedEntries)
{
    usedEntries = 0;
...
bool Storage::findEntry(nvs_opaque_iterator_t* it, const char* namespace_name)
{
    ...
    if (namespace_name != nullptr) {
        if(createOrOpenNamespace(namespace_name, false, it->nsIndex) != NVS_OK) {
            return false;
        }
    }
...
bool Storage::nextEntry(nvs_opaque_iterator_t* it)
{
    ...
    for (auto page = it->page; page != mPageManager.end(); ++page) {
        do {
            ...
            if(err == NVS_OK && isIterableItem(item) && !isMultipageBlob(item)) {
                fillEntryInfo(item, it->entry_info);
                it->page = page;
                return true;
            }
        } while (err != NVS_ERR_NVS_NOT_FOUND);
