...
public:
     ...
     */
    template<typename T>
    nvs_err_t set_item(const char *key, T value);
    virtual
    nvs_err_t set_string(const char *key, const char* value) = 0;
     ...
     */
    template<typename T>
    nvs_err_t get_item(const char *key, T &value);
     ...
     */
    virtual nvs_err_t set_blob(const char *key, const void* blob, size_t len) = 0;
     ...
     *             - ESP_ERR_NVS_INVALID_LENGTH if length is not sufficient to store data
     */
    virtual nvs_err_t get_string(const char *key, char* out_str, size_t len) = 0;
    virtual nvs_err_t get_blob(const char *key, void* out_blob, size_t len) = 0;
     ...
     *               error occurs.
     */
    virtual nvs_err_t get_item_size(ItemType datatype, const char *key, size_t &size) = 0;

    /**
     * @brief Erases an entry.
     */
    virtual nvs_err_t erase_item(const char* key) = 0;
     ...
     *               Return param used_entries will be filled 0.
     */
    virtual nvs_err_t get_used_entry_count(size_t& usedEntries) = 0;

protected:
    virtual nvs_err_t set_typed_item(ItemType datatype, const char *key, const void* data, size_t dataSize) = 0;

    virtual nvs_err_t get_typed_item(ItemType datatype, const char *key, void* data, size_t dataSize) = 0;
};
 ...
 *
 * @param err an optional pointer to an nvs_err_t result of the open operation, having the same meaning as the return
...
template<typename T>
nvs_err_t NVSHandle::set_item(const char *key, T value) {
    return set_typed_item(itemTypeOf(value), key, &value, sizeof(value));
}

template<typename T>
nvs_err_t NVSHandle::get_item(const char *key, T &value) {
