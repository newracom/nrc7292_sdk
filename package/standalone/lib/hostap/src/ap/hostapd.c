...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	if (flush_old_stations && !conf->start_disabled &&
		 ...
		 * operate. */
		wpa_dbg(hapd->msg_ctx, MSG_DEBUG,
			"Deauthentication all stations at BSS start");
		os_memset(addr, 0xff, ETH_ALEN);
						 ...
						 int err)
{
	...
	if (iface->freq) {
		...
		iface->freq_fractional = hostapd_hw_get_freq_fractional(hapd,
								 iface->conf->channel);
		wpa_printf(MSG_DEBUG, "Mode: %s  Channel: %d  "
			   "Frequency: %d.%d MHz",
			   hostapd_hw_mode_txt(iface->conf->hw_mode),
			   iface->conf->channel, iface->freq, iface->freq_fractional);
				      ...
				      struct hostapd_freq_params *old_params)
{
	...
	if (old_params &&
	    hostapd_set_freq_params(old_params, conf->hw_mode,
					...
					hostapd_hw_get_freq_fractional(hapd, conf->channel),
				    conf->channel,
					conf->enable_edmg, conf->edmg_channel,
					conf->ieee80211n,
				    conf->ieee80211ac, conf->ieee80211ax, conf->ieee80211ah,
