...
#define BEACON_TAIL_BUF_SIZE 512
	...
	if (hapd->iconf->ieee80211ah) {
		tail_len += 2 + sizeof(struct ieee80211_s1g_capabilities) +
...
#ifdef CONFIG_IEEE80211AH
	if (hapd->iconf->ieee80211ah) {
		tailpos = hostapd_eid_s1g_capabilities(hapd, tailpos);
		tailpos = hostapd_eid_s1g_operation(hapd, tailpos);
	}
#endif
	/* WPA / OSEN */
...
static int __ieee802_11_set_beacon(struct hostapd_data *hapd)
{
	...
	if (cmode &&
	    hostapd_set_freq_params(&freq, iconf->hw_mode, iface->freq,
				    ...
				    iconf->channel,
					iconf->enable_edmg, iconf->edmg_channel,
					iconf->ieee80211n, iconf->ieee80211ac,
					iconf->ieee80211ax, iconf->ieee80211ah,
