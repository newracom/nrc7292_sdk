 ...
 */
#define MQTT_SENSOR_TOPIC                  CLIENT_IDENTIFIER "/sensor"

/**
 * @brief Length of client MQTT topic.
 */
#define MQTT_SENSOR_TOPIC_LENGTH           ( ( uint16_t ) ( sizeof( MQTT_SENSOR_TOPIC ) - 1 ) )

/**
 * @brief The MQTT message published in this example.
 */
#define MQTT_EXAMPLE_MESSAGE                "Hello from SDK!"
...
/**
 * @brief Sends an MQTT SUBSCRIBE to subscribe to #MQTT_SENSOR_TOPIC
 ...
 * #MQTT_SENSOR_TOPIC defined at the top of the file.
 *
 * @param[in] pMqttContext MQTT context pointer.
 *
...
/**
 * @brief Sends an MQTT PUBLISH to #MQTT_SENSOR_TOPIC defined at
 * the top of the file.
 *
 * @param[in] pMqttContext MQTT context pointer.
 *
 ...
 * EXIT_FAILURE otherwise.
 */
static int publishToTopic( MQTTContext_t * pMqttContext, char *message, int size );
                                   ...
                                   uint16_t packetIdentifier )
{
    ...
    if( ( pPublishInfo->topicNameLength == MQTT_SENSOR_TOPIC_LENGTH ) &&
        ( 0 == strncmp( MQTT_SENSOR_TOPIC,
...
static int subscribeToTopic( MQTTContext_t * pMqttContext )
{
    ...
    pGlobalSubscriptionList[ 0 ].pTopicFilter = MQTT_SENSOR_TOPIC;
    pGlobalSubscriptionList[ 0 ].topicFilterLength = MQTT_SENSOR_TOPIC_LENGTH;
...
static int publishToTopic( MQTTContext_t * pMqttContext, char *message, int length )
{
    int returnStatus = EXIT_SUCCESS;
    MQTTStatus_t mqttStatus = MQTTSuccess;
    ...
    if( returnStatus == EXIT_SUCCESS ) {
        ...
        outgoingPublishPackets[ publishIndex ].pubInfo.pTopicName = MQTT_SENSOR_TOPIC;
        outgoingPublishPackets[ publishIndex ].pubInfo.topicNameLength = MQTT_SENSOR_TOPIC_LENGTH;
        outgoingPublishPackets[ publishIndex ].pubInfo.pPayload = message;
        outgoingPublishPackets[ publishIndex ].pubInfo.payloadLength = length;
...
int subscribe( MQTTContext_t * pMqttContext )
{
    ...
    if( returnStatus == EXIT_SUCCESS ) {
         ...
         * as specified in MQTT_SENSOR_TOPIC at the top of this file by sending a
...
int publish( MQTTContext_t * pMqttContext, char *message, int size )
{
	...
	LogInfo( ( "Sending Publish to the MQTT topic %.*s.",
			   MQTT_SENSOR_TOPIC_LENGTH,
			   MQTT_SENSOR_TOPIC ) );
	returnStatus = publishToTopic( pMqttContext, message, size );
...
int init_mqtt()
{
    ...
    if( returnStatus == EXIT_SUCCESS ) {
		LogInfo( ( "Starting the iteration...\n" ) );
		...
		LogInfo( ( "MQTT initialization completed successfully." ) );
    }
