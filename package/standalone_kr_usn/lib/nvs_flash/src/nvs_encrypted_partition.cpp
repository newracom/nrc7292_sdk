...
nvs_err_t NVSEncryptedPartition::init(nvs_sec_cfg_t* cfg)
{
    ...
    if (mbedtls_aes_xts_setkey_enc(&mEctxt, eky, 2 * NVS_KEY_SIZE * 8) != 0) {
        return NVS_ERR_NVS_XTS_CFG_FAILED;
    }
...
nvs_err_t NVSEncryptedPartition::read(size_t src_offset, void* dst, size_t size)
{
    ...
    * multi-entry data types. So length should always be equal to size of an entry.*/
    if (size != sizeof(Item)) return NVS_ERR_INVALID_SIZE;

    // read data
    nvs_err_t read_result = esp_partition_read(mESPPartition, src_offset, dst, size);
    if (read_result != NVS_OK) {
        return read_result;
    }
    ...
    if (mbedtls_aes_crypt_xts(&mDctxt, MBEDTLS_AES_DECRYPT, size, data_unit, destination, destination) != 0)  {
        return NVS_ERR_NVS_XTS_DECR_FAILED;
    }

    return NVS_OK;
}

nvs_err_t NVSEncryptedPartition::write(size_t addr, const void* src, size_t size)
{
    if (size % NVS_ENCRYPT_BLOCK_SIZE != 0) return NVS_ERR_INVALID_SIZE;

    // copy data to buffer for encryption
    uint8_t* buf = new (std::nothrow) uint8_t [size];

    if (!buf) return NVS_ERR_NO_MEM;
    ...
    for(uint8_t entry = 0; entry < (size/entrySize); entry++)
    {
        ...
        *addr_loc = relAddr + offset;
        if (mbedtls_aes_crypt_xts(&mEctxt,
                                  ...
                                  buf + offset) != 0)  {
            delete buf;
            return NVS_ERR_NVS_XTS_ENCR_FAILED;
        }
    }

    // write data
    nvs_err_t result = esp_partition_write(mESPPartition, addr, buf, size);
