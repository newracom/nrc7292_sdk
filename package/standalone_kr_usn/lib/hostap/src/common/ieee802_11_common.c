				...
				int show_errors)
{
	...
	for_each_element(elem, start, len) {
		...
		case WLAN_EID_DILS:
			...
			elems->dils_len = elen;
			break;
		case WLAN_EID_S1G_CAP:
			if (elen < sizeof(struct ieee80211_s1g_capabilities))
				break;
...
enum hostapd_hw_mode ieee80211_freq_to_chan2(int freq, int freq_fractional,
						u8 *channel)
{
	u8 op_class;

	return ieee80211_freq_to_channel_ext2(freq, freq_fractional, 0,
						CHANWIDTH_USE_HT, &op_class, channel);
} 

enum hostapd_hw_mode ieee80211_freq_to_channel_ext2(unsigned int freq,
						   unsigned int freq_fractional,
						   ...
						   u8 *op_class, u8 *channel)
{
	...
	if (freq < 1000) {
		/* TODOO:*/
		freq = freq * 10 + freq_fractional / 100;
		*channel = (freq - 9025) / 5 + 1;
		*op_class = 1;
...
enum hostapd_hw_mode ieee80211_freq_to_channel_ext(unsigned int freq,
						   ...
						   u8 *op_class, u8 *channel)
{
	return ieee80211_freq_to_channel_ext2(freq, 0, sec_channel, vht,
							op_class, channel);
}
