			  ...
			  struct nlattr *tb_freq[])
{
	...
	chan->allowed_bw = ~0;
	chan->dfs_cac_ms = 0;
	chan->freq_fractional = 0;
	if (tb_freq[NL80211_FREQUENCY_ATTR_FREQ_FRACTIONAL]) {
		chan->freq_fractional = 
			nla_get_u32(tb_freq[NL80211_FREQUENCY_ATTR_FREQ_FRACTIONAL]);

		if (ieee80211_freq_to_chan2(chan->freq, chan->freq_fractional, &channel)
							!= NUM_HOSTAPD_MODES)
			chan->chan = channel;
	}
	else if (ieee80211_freq_to_chan(chan->freq, &channel) != NUM_HOSTAPD_MODES)
		chan->chan = channel;
			  ...
			  struct hostapd_hw_modes *mode, struct nlattr *tb)
{
	static struct nla_policy freq_policy[NL80211_FREQUENCY_ATTR_MAX + 1] = {
		...
		[NL80211_FREQUENCY_ATTR_FREQ_FRACTIONAL] = { .type = NLA_FLAG },
	};
	int new_channels = 0;
				   ...
				   u16 num_modes)
{
	...
	for (i = 0; i < num_modes; i++) {
		...
		for (j = 0; j < mode->num_channels; j++) {
			...
			res = os_snprintf(pos, end - pos, " %d.%d%s%s%s",
					  chan->freq,
					  chan->freq_fractional,
					  (chan->flag & HOSTAPD_CHAN_DISABLED) ?
					  "[DISABLED]" : "",
